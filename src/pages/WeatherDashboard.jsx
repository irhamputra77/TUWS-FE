...
export default function WeatherDashboard() {
    ...
    return (
        ...
        >
            {/* === MOBILE (Android / kecil) === */}
            <div className="md:hidden w-full">
                <div className="mx-auto max-w-[700px] px-4 pb-8">
                    <header className="relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen mt-10 mb-10">
                        <div className="mx-auto max-w-[700px] px-4 text-center text-white select-none py-6">
                            ...
                            <div className="mt-2 flex items-end justify-center leading-none">
                                <span className="text-[56px] font-extrabold tracking-tight">
                                    {current.temp}
                                </span>
                    ...
                    {/* Kartu statistik + Forecast (forecast sekarang di atas kompas) */}
                    <section className="grid grid-cols-2 gap-4 max-[600px]:grid-cols-1">
                        ...
                        <div className="rounded-[22px] bg-white text-slate-900 shadow-xl px-4 py-4 w-full overflow-hidden">
                            <ul className="space-y-3 text-[12px]">
                                ...
                                <li className="flex items-start justify-between gap-3">
                                    <span className="flex items-center gap-2 font-semibold">
                                        <Sun className="h-4 w-4 opacity-70" /> Curah Hujan
                                    </span>
                                    <span className="font-extrabold tabular-nums">
                                        {current.rain_rate} mm
                                    </span>
                                </li>
                                <li className="flex items-start justify-between gap-3">
                                    <span className="flex items-center gap-2 font-semibold">
                                    ...
                                    <span className="inline-flex items-baseline justify-end gap-1 tabular-nums text-right whitespace-nowrap shrink-0">
                                        <span className="font-extrabold text-[13px]">
                                            {Number(current.Pressure).toFixed(2)}
                                        </span>
                                        <span className="font-semibold opacity-80 text-[12px]">
                                            Pa
                                        </span>
                                    </span>
                                </li>
                        ...
                        {/* Forecast khusus mobile (sekarang posisinya di sini, sebelum kompas) */}
                        <div className="rounded-[22px] border border-white/40 bg-white/10 backdrop-blur-xl shadow-xl px-4 py-3 text-white w-full">
                            <ForecastSwitcher value={view} onChange={setView} />
                            <ForecastList view={view} hourly={hourly} current={current} />
                        </div>
                    </section>

                    {/* Kompas dipindah ke bawah, setelah forecast */}
            ...
            {/* Forecast: hanya untuk DESKTOP (disembunyikan di mobile agar tidak dobel) */}
